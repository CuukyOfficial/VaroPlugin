	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			sender.sendMessage(Main.getPrefix() + Main.getColorCode()
					+ "/varo autostart §7set <Hour> <Minute> <Day> <Month> <Year>");
		...
		if (args[0].equalsIgnoreCase("set")) {
			...
			if (new GregorianCalendar().after(start)) {
				sender.sendMessage(Main.getPrefix() + "§7Das " + Main.getColorCode()
						+ "Datum §7darf nicht in der Vergangenheit sein!");
				return;
			}
			...
			if (Main.getGame().getAutoStart() == null) {
				sender.sendMessage(
						Main.getPrefix() + "§7Es wurde noch kein " + Main.getColorCode() + "Autostart §7festegelegt!");
				return;
			}
			...
			sender.sendMessage(
					Main.getPrefix() + "§7Der Start wurde um " + Main.getColorCode() + delay + " §7Minuten verzögert!");
		} else if (args[0].equalsIgnoreCase("info")) {
			if (Main.getGame().getAutoStart() == null)
				sender.sendMessage(Main.getPrefix() + "AutoStart nicht aktiv");
			else {
				sender.sendMessage(Main.getPrefix() + "AutoStart §aaktiv§7:");
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "Datum: §7"
						+ new SimpleDateFormat("dd.MM.yyyy HH.mm").format(Main.getGame().getAutoStart().getStart()));
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "AutoSort: §7"
						+ ConfigEntry.DO_SORT_AT_START.getValueAsBoolean());
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "AutoRandomteam: §7"
						+ ConfigEntry.DO_RANDOMTEAM_AT_START.getValueAsBoolean());
			}
		} else
