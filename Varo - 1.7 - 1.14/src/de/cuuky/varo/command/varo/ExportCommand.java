...
public class ExportCommand extends VaroCommand {
	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		if (args.length == 0) {
			...
			sender.sendMessage(
					Main.getPrefix() + Main.getColorCode() + "/varo export players | Exportiert alle Spieler");
		}
		...
		if (args[0].equalsIgnoreCase("players")) {
			...
			for (VaroPlayer player : VaroPlayer.getVaroPlayer()) {
				...
				if (player.getTeam() == null) {
					...
					yaml.set(player.getName() + ".youtubeLink",
							player.getStats().getYoutubeLink() == null ? "/" : player.getStats().getYoutubeLink());
					finished.add(player);
				}
			}
			...
			for (Team team : Team.getTeams()) {
				...
				for (VaroPlayer teamPl : team.getMember()) {
					...
					yaml.set(pref + ".member." + teamPl.getName() + ".youtubeLink",
							teamPl.getStats().getYoutubeLink() == null ? "/" : teamPl.getStats().getYoutubeLink());
				}
			}
