	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			sender.sendMessage(Main.getPrefix() + Main.getColorCode()
					+ "Tipp: §7Der /varo enchant Befehl blockt alle Enchantments, die auf deinem derzeitigen Item sind.");
			sender.sendMessage(Main.getPrefix());
			sender.sendMessage(
					Main.getPrefix() + "§7Dieser Command fügt die Sachen der Listen hinzu, die du in der Hand hältst.");
		...
		if (args[1].equalsIgnoreCase("list")) {
			sender.sendMessage(
					Main.getPrefix() + "Liste aller Items von " + Main.getColorCode() + list.getLocation() + "§7:");
