	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if (args[0].equalsIgnoreCase("build") || args[0].equalsIgnoreCase("create")) {
			...
			if (!uuid.contains(p.getUniqueId())) {
				sender.sendMessage(Main.getPrefix() + "§7Dieser Command wird eine " + Main.getColorCode()
						+ "Lobby §7um dich herum spawnen, was du nicht rückgängig machen kannst.");
				sender.sendMessage(Main.getPrefix() + "Führe diesen Command am besten " + Main.getColorCode()
						+ "Hoch in der Luft §7aus.");
				sender.sendMessage(Main.getPrefix() + "Gib den Command nochmal ein, um zu " + Main.getColorCode()
						+ "bestätigen§7.");
			...
			sender.sendMessage(
					Main.getPrefix() + "§7Location für die " + Main.getColorCode() + "Lobby §7erfolgreich gesetzt!");
			return;
		} else if (args[0].equalsIgnoreCase("removeSpawn") || args[0].equalsIgnoreCase("remove")) {
			Main.getGame().setLobby(null);
			sender.sendMessage(
					Main.getPrefix() + "§7Location für die " + Main.getColorCode() + "Lobby §7erfolgreich entfernt!");
			return;
		} else
