...
public class RandomTeamCommand extends VaroCommand {
	...
	public RandomTeamCommand() {
		super("randomteam", "Gibt allen Spielern, die noch kein Team haben, einen zufälligen Teampartner mit Größe",
				"varo.randomteam", "rt");
	}
	...
	public void onCommand(CommandSender sender, VaroPlayer vpsender, Command cmd, String label, String[] args) {
		...
		if (teamsize >= 2) {
			...
			for (VaroPlayer vp : VaroPlayer.getOnlinePlayer()) {
				...
				if (teamMember.size() != teamsize)
					vp.getPlayer().sendMessage(Main.getPrefix() + "§7Für dich wurden nicht genug" + Main.getColorCode()
							+ " Teampartner §7gefunden!");
		...
		sender.sendMessage(
				Main.getPrefix() + "§7Alle Spieler sind nun in " + Main.getColorCode() + teamsize + "§7'(n)er Teams!");
		if (ConfigEntry.TEAMREQUESTS.getValueAsBoolean())
			Bukkit.broadcastMessage("§7Spieler, die schon ein " + Main.getColorCode()
					+ "Team §7hatten, wurden in kein Team gesetzt, da das" + Main.getColorCode()
					+ " TeamRequestor System §7aktiv ist.");
	}
}
