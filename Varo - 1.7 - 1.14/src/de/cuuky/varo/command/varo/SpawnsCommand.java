...
public class SpawnsCommand extends VaroCommand {
	...
	public SpawnsCommand() {
		super("spawns", "Hauptbefehl für die Spawns, in welchen die Spieler spawnen", "varo.spawns", "spawnholes",
				"holes");
	}

	@SuppressWarnings("unchecked")
	@Override
	public void onCommand(CommandSender sender, final VaroPlayer vp, final Command cmd, final String label,
			final String[] args) {
		if (args.length == 0) {
			...
			sender.sendMessage(Main.getPrefix() + Main.getColorCode()
					+ "/varo spawns§7 remove [Zahl/Spieler] - (Löscht nur den Eintrag)");
			sender.sendMessage(Main.getPrefix() + Main.getColorCode()
					+ "/varo spawns§7 delete [Zahl/Spieler] - (Löscht auch Spawn in der Welt)");
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo spawns§7 list");
			sender.sendMessage(Main.getPrefix() + Main.getColorCode()
					+ "/varo spawns§7 generate <radius> <amount> [Half-Step-Materiall] [Side-Block-Material]");
			sender.sendMessage(
					Main.getPrefix() + Main.getColorCode() + "Example: §7/varo spawns generate 30 40 STONE_SLAB");
			return;
		}
		...
		if (args[0].equalsIgnoreCase("generate")) {
			...
			try {
				new SpawnGenerator(((Player) sender).getLocation(), Integer.valueOf(args[1]), Integer.valueOf(args[2]),
						material, sideBlockMaterial);
			} catch (Exception e) {
			...
			sender.sendMessage(Main.getPrefix() + "§7Die Spawns wurden mit der Anzahl " + Main.getColorCode() + args[2]
					+ "§7, dem Radius " + Main.getColorCode() + args[1] + "§7, dem Block-Material "
					+ Main.getColorCode() + (args.length >= 4 ? args[3] : "STONE_BRICK_SLAB")
					+ " §7und dem Seitenblock-Material " + Main.getColorCode() + (args.length >= 5 ? args[4] : "DIRT")
					+ " §7generiert!");
			return;
			...
			if (args.length == 1) {
				...
				player.sendMessage(Main.getPrefix() + Main.getColorCode() + "Spawn " + spawn.getNumber()
						+ " §7erfolgreich erstellt!");
				...
				if (spawnNumber != -1) {
					...
					if (oldSpawn != null) {
						oldSpawn.delete();
						sender.sendMessage(Main.getPrefix() + "Der alte Spawn mit der ID " + Main.getColorCode()
								+ spawnNumber + " §7wurde entfernt, um für den neuen Platz zu machen.");
					}

					final Spawn spawn = new Spawn(SpawnType.NUMBERS, spawnNumber, player.getLocation());
					sender.sendMessage(
							Main.getPrefix() + "Spawn " + Main.getColorCode() + spawn.getNumber() + " §7gesetzt!");
				} else {
					...
					sender.sendMessage(Main.getPrefix() + "Spielerspawn " + Main.getColorCode() + spawn.getNumber()
							+ " §7für den Spieler " + Main.getColorCode() + spawn.getPlayer().getName()
							+ " §7gesetzt!");
				}
			} else
			...
			if (spawnNumber != -1) {
				...
				if (spawn == null) {
					sender.sendMessage(Main.getPrefix() + "Spawn mit der ID" + Main.getColorCode() + spawnNumber
							+ " nicht gefunden!");
					return;
				}
				...
				if (args[0].equalsIgnoreCase("remove"))
					...
					spawn.delete();
				sender.sendMessage(
						Main.getPrefix() + "Spawn " + Main.getColorCode() + spawn.getNumber() + " §7entfernt!");
			} else {
				VaroPlayer varoplayer = VaroPlayer.getPlayer(args[1]);
				if (varoplayer == null) {
					sender.sendMessage(Main.getPrefix() + "Spieler oder Zahl nicht gültig!");
				...
				if (spawn == null) {
					sender.sendMessage(Main.getPrefix() + "Spawn von dem Spieler " + Main.getColorCode()
							+ varoplayer.getName() + " nicht gefunden!");
					return;
				}

				sender.sendMessage(
						Main.getPrefix() + "Spawn von " + Main.getColorCode() + varoplayer.getName() + " §7entfernt!");
			}
			...
			for (Spawn spawn : Spawn.getSpawns()) {
				...
				sender.sendMessage(
						Main.getPrefix() + "§7Location: X:" + Main.getColorCode() + spawn.getLocation().getBlockX()
								+ " §7Y: " + Main.getColorCode() + spawn.getLocation().getBlockY() + " §7Z: "
								+ Main.getColorCode() + spawn.getLocation().getBlockZ());
				sender.sendMessage(Main.getPrefix() + "§7Spieler: " + Main.getColorCode()
						+ (spawn.getPlayer() != null ? spawn.getPlayer().getName() : "Keinen"));
				sender.sendMessage(Main.getPrefix());
			}
			return;
		} else
