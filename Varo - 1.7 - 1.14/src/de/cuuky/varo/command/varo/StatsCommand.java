...
public class StatsCommand extends VaroCommand {
	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		if (args.length == 0) {
			...
			sender.sendMessage(
					Main.getPrefix() + Main.getColorCode() + label + " stats set §7<Spieler/@a> <Stat> <Value>");
			...
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + label
					+ " stats odvReset §7<Spieler/@a> | Resettet alles außer Kills, Wins, Rank, Team und YT-Link");
			return;
		}
		...
		if (args[0].equalsIgnoreCase("set") || args[0].equalsIgnoreCase("remove")) {
			...
			if (args[0].equalsIgnoreCase("set")) {
				if (args.length != 4) {
					sender.sendMessage(Main.getPrefix() + Main.getColorCode() + label
							+ " stats set §7<Spieler/@a> <Stat> <Value>");
					return;
				}
			} else {
				if (args.length != 3) {
					sender.sendMessage(
							Main.getPrefix() + Main.getColorCode() + label + " stats remove §7<Spieler/@a> <Stat>");
					return;
				}
			}

			if (type == null) {
				sender.sendMessage(Main.getPrefix() + "Stat '" + Main.getColorCode() + args[2]
						+ "§7' konnte nicht gefunden werden!");
				return;
			}
			...
			if (target == null) {
				if (args[0].equalsIgnoreCase("set")) {
					...
					sender.sendMessage(Main.getPrefix() + "Stat '" + Main.getColorCode() + type.getArg()
							+ "§7' für alle Spieler erfolgreich auf '" + Main.getColorCode() + args[3]
							+ "§7' gesetzt!");
				} else {
					for (VaroPlayer all : VaroPlayer.getVaroPlayer())
						type.remove(all);
					...
					sender.sendMessage(Main.getPrefix() + "Stat '" + Main.getColorCode() + type.getArg()
							+ "§7' für alle Spieler erfolgreich zurückgesetzt/entfernt!");
				}
			...
			if (args[0].equalsIgnoreCase("set")) {
				if (type.execute(args[3], target, sender))
					sender.sendMessage(Main.getPrefix() + "Stat '" + Main.getColorCode() + type.getArg() + "§7' für '"
							+ Main.getColorCode() + target.getName() + "§7' erfolgreich auf '" + Main.getColorCode()
							+ args[3] + "§7' gesetzt!");
			} else {
				type.remove(target);
				sender.sendMessage(Main.getPrefix() + "Stat '" + Main.getColorCode() + type.getArg() + "§7' für '"
						+ Main.getColorCode() + target.getName() + "§7' erfolgreich zurückgesetzt/entfernt!");
			}
			...
			sender.sendMessage(Main.getPrefix() + "Spieler '" + Main.getColorCode() + target.getName()
					+ "§7' erfolgreich zurückgesetzt!");
			...
			sender.sendMessage(Main.getPrefix() + "Spieler '" + Main.getColorCode() + target.getName()
					+ "§7' erfolgreich ODV-zurückgesetzt!");
		} else
			sender.sendMessage(
	...
	public enum SetArgumentType {
		KILLS("kills"), SESSIONS("sessions"), EPISODES_PLAYED("episodesplayed"), PRE_PRODUCED("preproduced"),
		RANK("rank"), COUNTDOWN("countdown"), WINS("wins"), YOUTUBE_LINK("youtubelink"), PLAYER_STATE("playerstate");
		...
		public boolean execute(String value, VaroPlayer vp, CommandSender sender) {
			try {
				...
				sender.sendMessage(Main.getPrefix() + "§7Der Wert '" + Main.getColorCode() + value
						+ "§7' §7konnte nicht für " + this.toString() + " gesetzt werden!");
				return false;
			}
			return true;
		}
