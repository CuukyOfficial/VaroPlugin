...
public class StrikeCommand extends VaroCommand {
	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		if (args.length == 0) {
			...
			sender.sendMessage(
					Main.getPrefix() + Main.getColorCode() + "/varo strike remove §7<Player> <StrikeNummer>");
		...
		if (VaroPlayer.getPlayer(args[0]) != null) {
			...
			Strike strike = new Strike(reason, varoPlayer,
			...
			if (varoPlayer.getStats().getStrikes().size() < 1) {
				sender.sendMessage(
						Main.getPrefix() + Main.getColorCode() + varoPlayer.getName() + " §7hat keine Strikes!");
				return;
			}
			...
			sender.sendMessage(Main.getPrefix() + "§7Du hast " + Main.getColorCode() + vp.getName()
					+ " §7einen Strike entfernt! Er hat noch " + Main.getColorCode() + vp.getStats().getStrikes().size()
					+ " §7strikes!");
			...
			for (int i = 1; i < varoPlayer.getStats().getStrikes().size(); i++) {
				...
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "Acquired: §7"
						+ new SimpleDateFormat("dd:MM:yyy HH:mm").format(strike.getAcquiredDate()));
			}
		} else
