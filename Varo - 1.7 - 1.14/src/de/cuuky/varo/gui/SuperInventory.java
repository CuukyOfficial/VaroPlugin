...
public abstract class SuperInventory {
	...
	private void setSwitcher() {
		inv.setItem(modifier.get(2),
				new ItemBuilder().displayname(getBack())
						.itemstack(getBack().equals("§4Zurück") ? new ItemStack(Materials.STONE_BUTTON.parseMaterial())
								: Materials.REDSTONE.parseItem())
						.build());
		if (!hasMorePages)
			return;
	...
	private void fillSpace() {
		...
		for (int i = 0; i < inv.getSize(); i++)
			if (inv.getItem(i) == null)
				inv.setItem(i,
						new ItemBuilder().displayname("§c").itemstack(
								new ItemStack(Materials.BLACK_STAINED_GLASS_PANE.parseMaterial(), 1, (short) 15))
								.build());
	}
