...
public class StrikeListGUI extends SuperInventory {
	...
	public boolean onOpen() {
		...
		for (int i = 0; i != getSize(); i++) {
			...
			linkItemTo(i, new ItemBuilder().displayname("§c" + (i + 1)).itemstack(new ItemStack(Material.PAPER))
					.lore(new String[] { "§7Reason: §c" + strike.getReason(), "§7Striker: §c" + strike.getStriker(),
							"§7Date: §c"
									+ new SimpleDateFormat("dd.MM.yyyy HH:mm:ss").format(strike.getAcquiredDate()) })
					.build(), new Runnable() {
