...
public class SignChangeListener implements Listener {
	...
	public void onSignChange(SignChangeEvent e) {
		...
		if (e.getBlock().getType() != Materials.SIGN.parseMaterial()
				&& e.getBlock().getType() != Materials.WALL_SIGN.parseMaterial())
			return;
		...
		if (attached.getState() instanceof Chest) {
			...
			if (sorted.size() >= ConfigEntry.PLAYER_CHEST_LIMIT.getValueAsInt()
					|| secChest != null && sorted.size() + 1 >= ConfigEntry.PLAYER_CHEST_LIMIT.getValueAsInt()) {
				p.sendMessage(Main.getPrefix() + "§7Die maximale Anzahl an gesetzten Kisten für das Team "
						+ Main.getColorCode() + player.getTeam().getName() + " §7wurde bereits §7erreicht! (Anzahl: §6"
						+ sorted.size() + " §7Max: §6" + ConfigEntry.PLAYER_CHEST_LIMIT.getValueAsInt() + "§7)");
				e.setCancelled(true);
				return;
			}

			if (VaroSaveable.getByLocation(chest.getLocation()) != null
					|| secChest != null && VaroSaveable.getByLocation(secChest.getLocation()) != null) {
			...
			e.setLine(2, Main.getColorCode()
					+ (player.getTeam() != null ? player.getTeam().getDisplay() : player.getName()));
			e.setLine(3, "§8--------------");
			...
			if (ConfigEntry.PLAYER_FURNACE_LIMIT.getValueAsInt() == 0) {
				e.getPlayer()
						.sendMessage(Main.getPrefix() + "§7Die Furnacesicherung wurde in der Config §7deaktiviert!");
				return;
			}
			...
			if (VaroSaveable.getByLocation(furnace.getLocation()) != null) {
				p.sendMessage(Main.getPrefix() + "§7Diese " + ConfigEntry.PROJECTNAME_COLORCODE.getValueAsString()
						+ " Furnace §7ist bereits gesichert!");
				e.setCancelled(true);
				return;
			}
			...
			if (ConfigEntry.PLAYER_FURNACE_LIMIT.isIntActivated())
				if (sorted.size() >= ConfigEntry.PLAYER_FURNACE_LIMIT.getValueAsInt()) {
					p.sendMessage(Main.getPrefix() + "§7Die maximale Anzahl an gesetzten Furnaces für das Team "
							+ Main.getProjectName() + " " + player.getTeam().getDisplay()
							+ " §7wurde bereits §7erreicht! (Anzahl: §6" + sorted.size() + " §7Max: §6"
							+ ConfigEntry.PLAYER_FURNACE_LIMIT.getValueAsInt() + "§7)");
					e.setCancelled(true);
					return;
				}
