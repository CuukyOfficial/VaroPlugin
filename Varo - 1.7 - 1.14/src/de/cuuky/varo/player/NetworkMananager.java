...
public class NetworkMananager {
	...
	public NetworkMananager(Player player) {
		...
		try {
			...
			sendPacketMethod = connection.getClass().getDeclaredMethod("sendPacket",
	...
	public void sendTablist() {
		...
		try {
			Object tabheader = ioBaseChat.getDeclaredMethod("a", String.class).invoke(ioBaseChat,
					"{\"text\": \"" + ConfigMessages.TABLIST_HEADER.getValue() + "\"}");
			Object tabfooter = ioBaseChat.getDeclaredMethod("a", String.class).invoke(ioBaseChat,
					"{\"text\": \"" + ConfigMessages.TABLIST_FOOTER.getValue() + "\"}");

			if (tablist == null) {
				tablist = Class.forName(VersionUtils.getNmsClass() + ".PacketPlayOutPlayerListHeaderFooter")
						.newInstance();
	...
	public void sendActionbar(String message) {
		...
		try {
			Object barchat = ioBaseChat.getDeclaredMethod("a", String.class).invoke(ioBaseChat,
					"{\"text\": \"" + message + "\"}");
			...
			if (chatMessageTypeClass == null)
				...
				packet = packetChatClass.getConstructor(ioBase, chatMessageTypeClass).newInstance(barchat,
						chatMessageTypeClass.getDeclaredField("GAME_INFO").get(null));
	...
	public void sendTitle(String header, String footer) {
		...
		try {
			Object titleHeader = ioBaseChat.getDeclaredMethod("a", String.class).invoke(ioBaseChat,
					"{\"text\": \"" + header + "\"}");
			Object titleFooter = ioBaseChat.getDeclaredMethod("a", String.class).invoke(ioBaseChat,
					"{\"text\": \"" + footer + "\"}");

			Object headerPacket = titleConstructor.newInstance(enumTitleClass.getDeclaredField("TITLE").get(null),
					titleHeader, 0, 2, 0);
			Object footerPacket = titleConstructor.newInstance(enumTitleClass.getDeclaredField("SUBTITLE").get(null),
					titleFooter, 0, 2, 0);
	...
	public void setAttributeSpeed(double value) {
		...
		try {
			...
			Object attributeInstance = player.getClass().getMethod("getAttribute", genericSpeedType.getClass())
					.invoke(player, genericSpeedType);
	...
	public void sendLinkedMessage(String message, String link) {
		try {
			...
			Object text = ioBaseChat.getMethod("a", String.class).invoke(ioBaseChat, "{text: '" + message
					+ "', color: 'white', clickEvent: {\"action\": \"open_url\" , value: \"" + link + "\"}}");
