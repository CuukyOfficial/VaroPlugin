...
public class VaroSerializeHandlerOld {
	 ...
	 * this
	 * 
	 * OLD CODE -> Removed soon
	 */

	private static final String NULL_REPLACE = "nullReplace";

	private static List<VaroSerializeHandlerOld> handler;
	...
	static {
		handler = new ArrayList<VaroSerializeHandlerOld>();
	...
	public VaroSerializeHandlerOld(Class<? extends VaroSerializeable> clazz, String fileName) {
		this.clazz = clazz;
	...
	public VaroSerializeHandlerOld(Class<? extends VaroSerializeable> clazz) {
		this.clazz = clazz;
		...
		sectionLoop: for(String s : section.getKeys(true)) {
			...
			if(field != null) {
				...
				try {
					...
					VaroSerializeHandlerOld handl = getHandler((Class<?>) field.getType());
	...
	public void save(String saveUnder, VaroSerializeable instance, YamlConfiguration saveTo) {
		VaroSerializeHandlerOld handler = getHandler(instance.getClass());
		...
		for(String fieldIdent : list1) {
			try {
				...
				if(obj != null) {
						...
						if(!list.isEmpty())
							if(list.get(0) instanceof VaroSerializeable) {
								for(int i = 0; i < list.size(); i++) {
									...
									VaroSerializeHandlerOld handl = getHandler((Class<?>) listObject.getClass());
									if(handl != null) {
					...
					VaroSerializeHandlerOld handl = getHandler((Class<?>) obj.getClass());
					if(handl != null) {
	...
	private static VaroSerializeHandlerOld getHandler(Class<?> clazz) {
		for(VaroSerializeHandlerOld handl : handler)
	...
	public static void registerClass(Class<? extends VaroSerializeable> clazz) {
		new VaroSerializeHandlerOld(clazz);
	}
