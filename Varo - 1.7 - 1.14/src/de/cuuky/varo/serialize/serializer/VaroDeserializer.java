...
public class VaroDeserializer extends VaroSerializeHandler {
	...
	// ArrayList<String> handled = new ArrayList<String>();
	//
	// sectionLoop: for(String path : section.getKeys(true)) {
	// for(String handl : handled)
	// if(path.contains(handl))
	// continue sectionLoop;
	//
	// Field field = object.getFieldLoader().getFields().get(path);
	// if(field != null) {
	// if(instance == null)
	// try {
	// instance = object.getClazz().newInstance();
	...
	// if(handl != null && obj != null) {
	// handled.add(path);
	// VaroDeserializer des = new VaroDeserializer((MemorySection) obj, handl);
	// field.set(instance, des.deserialize());
	// continue;
	// }
	//
	// for(VaroSerialize vs : VaroSerialize.getSerializes()) {
	// switch(vs.getLoopType()) {
	// case CONTINUE:
	// handled.add(path);
	// field.set(instance, vs.deserialize(field, obj, section, path, handl));
	// break;
	// case LOOP:
	// obj = vs.deserialize(field, obj);
	// break;
	// case LOOP_HANDLE:
	// handled.add(path);
	// obj = vs.deserialize(field, obj, section, path, handl);
	// break;
	// }
	// }
	//
	...
	// ExceptionInInitializerError | NullPointerException e) {
	// e.printStackTrace();
	// continue;
	// }
	// } else
	// System.out.println(Main.getConsolePrefix() + "Could not deserialize field
