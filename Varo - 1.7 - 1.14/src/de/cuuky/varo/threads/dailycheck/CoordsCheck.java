...
public class CoordsCheck extends Checker {
	...
	public void check() {
		...
		if (new Date().after(
				DateUtils.addDays(Main.getGame().getLastCoordsPost(), ConfigEntry.POST_COORDS_DAYS.getValueAsInt()))) {
			String post = "";
			for (VaroPlayer vp : VaroPlayer.getAlivePlayer())
				post = post + (post.isEmpty() ? "Liste der Koordinaten aller Spieler:\n\n" : "\n") + vp.getName()
						+ (vp.getTeam() != null ? " (#" + vp.getTeam().getName() + ")" : "") + ": "
						+ (vp.getStats().getLastLocation() != null
								? new LocationFormatter("X:x Y:y Z:z in world").format(vp.getStats().getLastLocation())
								: "/");
