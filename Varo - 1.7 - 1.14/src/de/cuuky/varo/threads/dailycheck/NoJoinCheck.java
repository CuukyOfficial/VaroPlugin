...
public class NoJoinCheck extends Checker {
	...
	public void check() {
		...
		for (VaroPlayer vp : VaroPlayer.getAlivePlayer()) {
			...
			if (vp.getStats().getLastJoined()
					.before(DateUtils.addDays(current, -ConfigEntry.NO_ACTIVITY_DAYS.getValueAsInt()))) {
				new Alert(AlertType.NO_JOIN,
						vp.getName() + " hat die Anzahl an maximal inaktiven Tagen überschritten!");
				if (ConfigEntry.STRIKE_ON_NO_ACTIVITY.getValueAsBoolean()) {
					Main.getLoggerMaster().getEventLogger().println(LogType.ALERT,
							ConfigMessages.ALERT_NOT_JOIN_STRIKE.getValue(vp)
									.replace("%days%", String.valueOf(
											(int) getDateDiff(vp.getStats().getLastJoined(), current, TimeUnit.DAYS)))
									.replace("%player%", vp.getName()));

					vp.getStats().addStrike(new Strike("Activity Strike", vp, "CONSOLE"));
				} else
					Main.getLoggerMaster().getEventLogger().println(LogType.ALERT,
							ConfigMessages.ALERT_NOT_JOIN.getValue(vp)
									.replace("%days%", String.valueOf(
											(int) getDateDiff(vp.getStats().getLastJoined(), current, TimeUnit.DAYS)))
									.replace("%player%", vp.getName()));
			}
