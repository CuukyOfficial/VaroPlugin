...
public class YouTubeCheck extends Checker {
	...
	public void check() {
		for (VaroPlayer vp : VaroPlayer.getAlivePlayer()) {
			...
			if (videos == null)
				new Alert(AlertType.NO_YOUTUBE_UPLOAD,
						"Die Videos von " + vp.getName() + " konnten nicht geladen werden!");
	...
	private ArrayList<YouTubeVideo> loadVideos(String url, VaroPlayer player) {
		...
		try {
			...
			connection.setRequestProperty("User-Agent",
					"Mozilla/5.0 (Windows NT10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko)	Chrome/70.0.3538.67 Safari/537.36");
		...
		for (String line : lines) {
			if (line.contains("yt-lockup-title")) {
				try {
					...
					if (!videoTitle.toLowerCase()
							.contains(ConfigEntry.YOUTUBE_VIDEO_IDENTIFIER.getValueAsString().toLowerCase()))
						continue;
