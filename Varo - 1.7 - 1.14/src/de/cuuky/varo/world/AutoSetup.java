...
public class AutoSetup {
	...
	public AutoSetup() {
		...
		int x = 0, z = 0;
		while (!SpawnChecker.checkSpawns(world, x, z, ConfigEntry.AUTOSETUP_SPAWNS_RADIUS.getValueAsInt(),
				ConfigEntry.AUTOSETUP_SPAWNS_AMOUNT.getValueAsInt())) {
			x += 100;
			z += 100;
		}
		...
		portal: if (ConfigEntry.AUTOSETUP_PORTAL_ENABLED.getValueAsBoolean()) {
			...
			int width = ConfigEntry.AUTOSETUP_PORTAL_WIDTH.getValueAsInt(),
					height = ConfigEntry.AUTOSETUP_PORTAL_HEIGHT.getValueAsInt();
		...
		if (ConfigEntry.AUTOSETUP_LOBBY_ENABLED.getValueAsBoolean()) {
			...
			if (!file.exists())
				new LobbyGenerator(lobby, ConfigEntry.AUTOSETUP_LOBBY_HEIGHT.getValueAsInt(),
						ConfigEntry.AUTOSETUP_LOBBY_SIZE.getValueAsInt());
			else
		...
		if (ConfigEntry.AUTOSETUP_BORDER.isIntActivated()) {
			try {
				...
				if (method != null) {
					...
					border.getClass().getDeclaredMethod("setSize", double.class).invoke(border,
							ConfigEntry.AUTOSETUP_BORDER.getValueAsInt());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		new SpawnGenerator(middle, ConfigEntry.AUTOSETUP_SPAWNS_RADIUS.getValueAsInt(),
				ConfigEntry.AUTOSETUP_SPAWNS_AMOUNT.getValueAsInt(),
				ConfigEntry.AUTOSETUP_SPAWNS_BLOCKID.getValueAsString(),
				ConfigEntry.AUTOSETUP_SPAWNS_SIDEBLOCKID.getValueAsString());
