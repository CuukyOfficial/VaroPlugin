	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(xrayAvailable == 0) {
			...
			if(enabled == null || engineMode == null) {
				sender.sendMessage(Main.getPrefix() + "ßcEs gab einen Fehler mit dem Anti-Xray-System.");
				sender.sendMessage(Main.getPrefix() + "Dies kann daran liegen, dass du eine nicht-unterst√ºtzte Serverversion benutzt.");
				return false;
			}
		...
		if(xrayAvailable == 2) {
			sender.sendMessage(Main.getPrefix() + "In deiner Serverversion ist kein Anti-Xray verf√ºgbar.");
			return false;
		}

		if(args.length != 1 || (!args[0].equalsIgnoreCase("on") && !args[0].equalsIgnoreCase("off"))) {
			sender.sendMessage(Main.getPrefix() + "ß7Anti-Xray momentan ßl" + (antiXrayActivated ? "aktiviert" : "deaktiviert"));
			sender.sendMessage(Main.getPrefix() + "ß7/antixray on - Aktiviert den Schutz vor X-Ray");
			sender.sendMessage(Main.getPrefix() + "ß7/antixray off - Deaktiviert den Schutz vor X-Ray");
			return false;
		}

		if(args[0].equalsIgnoreCase("on")) {
			if(antiXrayActivated) {
				sender.sendMessage(Main.getPrefix() + "ß7Das Anti-Xray ist bereits aktiviert.");
				return false;
			} else {
				if(xrayAvailable == 0) {
					...
					try {
						...
						sender.sendMessage(Main.getPrefix() + "ßcFehler: ß7Das Anti-Xray konnte nicht aktiviert werden.");
						return false;
					}
				...
				sender.sendMessage(Main.getPrefix() + "ß7Das Anti-Xray wurde aktiviert.");
			}
		} else if(args[0].equalsIgnoreCase("off")) {
			if(!antiXrayActivated) {
				sender.sendMessage(Main.getPrefix() + "ß7Das Anti-Xray ist bereits deaktiviert.");
				return false;
			} else {
				if(xrayAvailable == 0) {
					spigotConfig.set("world-settings.default.anti-xray.enabled", true);
					...
					try {
						...
						sender.sendMessage(Main.getPrefix() + "ßcFehler: ß7Das Anti-Xray konnte nicht deaktiviert werden.");
						return false;
					}
				} else if(xrayAvailable == 1) {
				...
				sender.sendMessage(Main.getPrefix() + "ß7Das Anti-Xray wurde deaktiviert.");
			}
		}
