...
public class PingCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String ping, String[] args) {
		if(args.length == 0) {
			...
			if(p == null) {
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + args[0] + " ß7nicht gefunden!");
				return false;
			}

			sender.sendMessage(Main.getPrefix() + "ß7Der Ping von " + Main.getColorCode() + args[0] + " ß7betr√§gt " + Main.getColorCode() + String.valueOf(VaroPlayer.getPlayer(p).getNetworkManager().getPing()) + "msß7!");
		} else
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/ping ß7[Player]");
