...
public class BackpackCommand extends VaroCommand {
	...
	public BackpackCommand() {
		super("backpack", "√ñffnet das Backpack von dir oder einem Spieler", null, "bp");
	}
		...
		if(ConfigEntry.BACKPACK_PLAYER_ENABLED.getValueAsBoolean() && ConfigEntry.BACKPACK_TEAM_ENABLED.getValueAsBoolean()) {
			if(args.length == 0 || (!args[0].equalsIgnoreCase("player") && !args[0].equalsIgnoreCase("team"))) {
				...
				if(vp.getPlayer().isOp()) {
					sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo bp player ß7[Player]");
					sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo bp team ß7[Team]");
				} else {
			...
			sender.sendMessage(Main.getPrefix() + "Rucks√§cke sind nicht aktiviert!");
		}
	}
}