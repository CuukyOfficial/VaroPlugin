	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if(args[1].equalsIgnoreCase("add")) {
			...
			if(multipleAdd.contains(list)) {
				...
				sender.sendMessage(Main.getPrefix() + "Item erfolgreich " + String.valueOf(Anzahl) + " mal zu " + list.getLocation() + " hinzugefÃ¼gt!");
			} else {
				if(list.hasItem(item)) {
					...
					sender.sendMessage(Main.getPrefix() + "Item erfolgreich zu " + list.getLocation() + " hinzugefÃ¼gt!");
					list.addItem(item);
				}
			}
