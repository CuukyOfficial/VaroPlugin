...
public class PresetCommand extends VaroCommand {
	...
	public PresetCommand() {
		super("preset", "Command f√ºr die Presets", "varo.preset", "presettings", "presets");
	}

	@Override
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		if(args.length == 0) {
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo preset ß7load <PresetPath>");
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo preset ß7save <PresetPath>");
			sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo preset ß7list");
			return;
		}
		...
		if(args[0].equalsIgnoreCase("load")) {
			...
			sender.sendMessage(Main.getPrefix() + "ßlListe aller Presets:");
