...
public class ScoreboardCommand extends VaroCommand {
	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if(!ConfigSetting.SCOREBOARD.getValueAsBoolean()) {
			sender.sendMessage(Main.getPrefix() + Main.getLanguageManager().getValue(LanguageDE.VARO_COMMANDS_SCOREBOARD_DEACTIVATED, vp));
			return;
		}

		if(vp.getStats().isShowScoreboard()) {
			vp.getPlayer().getScoreboard().clearSlot(DisplaySlot.SIDEBAR);
			vp.sendMessage(Main.getPrefix() + Main.getLanguageManager().getValue(LanguageDE.VARO_COMMANDS_SCOREBOARD_DISABLED, vp));
			...
			vp.sendMessage(Main.getPrefix() + Main.getLanguageManager().getValue(LanguageDE.VARO_COMMANDS_SCOREBOARD_ENABLED, vp));
