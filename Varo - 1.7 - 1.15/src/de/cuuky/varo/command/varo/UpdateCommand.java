...
public class UpdateCommand extends VaroCommand {
	...
	private void update(CommandSender sender, VaroUpdateResultSet resultSet) {
		...
		try {
			...
			sender.sendMessage(Main.getPrefix() + "ßcEs bgab einen kritischen Fehler beim Download des Plugins.");
			sender.sendMessage(Main.getPrefix() + "ß7Empfohlen wird ein manuelles Updaten des Plugins: https://www.spigotmc.org/resources/71075/");
		...
		if(resetOldDirectory) {
			System.out.println("Das Verzeichnis der alten Pluginversion wird gel√∂scht.");
		...
		if(this.pluginNameChanged) {
			System.out.println("Da sich der Pluginname ver√§ndert hat, wird die alte Pluginversion gel√∂scht.");
	...
	public void onCommand(CommandSender sender, VaroPlayer vp, Command cmd, String label, String[] args) {
		...
		if(args.length == 0 || (!args[0].equalsIgnoreCase("normal") && !args[0].equalsIgnoreCase("reset"))) {
			if(result == UpdateResult.UPDATE_AVAILABLE) {
				sender.sendMessage(Main.getPrefix() + "ßc Es existiert eine neuere Version: " + updateVersion);
				sender.sendMessage("");
				sender.sendMessage(Main.getPrefix() + "ß7ßlUpdate Befehle:");
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo update normal ß7- Updated die Version, aber beh√§lt alle Daten");
				sender.sendMessage(Main.getPrefix() + Main.getColorCode() + "/varo update reset ß7- Updated die Version und l√∂scht alle Daten");
				sender.sendMessage(Main.getPrefix() + "ßcWichtig: ß7Der Updater spiget.org hat manchmal eine alte Version als Download. Falls sich nach dem Update die Version nicht ge√§ndert haben sollte, musst du manuell updaten.");
			} else {
		...
		if(result == UpdateResult.UPDATE_AVAILABLE) {
			sender.sendMessage(Main.getPrefix() + "ß7Update wird installiert...");
			sender.sendMessage(Main.getPrefix() + "ß7Unter Umst√§nden wird nicht die neuste Version heruntergeladen, sollte dies der Fall sein, installieren die neue Version bitte manuell.");
			update(sender, resultSet);
		} else {
			sender.sendMessage(Main.getPrefix() + "ß7Das Plugin ist bereits auf dem neuesten Stand!");
		}
	}
}