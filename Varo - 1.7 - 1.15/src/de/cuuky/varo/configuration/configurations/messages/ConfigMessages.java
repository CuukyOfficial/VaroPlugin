...
public enum ConfigMessages implements SectionEntry {
	...
	ALERT_FIRST_STRIKE(ConfigMessageSection.ALERTS, "BOTS_ALERT.firstStrike", "%player% hat nun einen Strike. Der Strike wurde von %striker% gegeben. Begründung: %reason%\nAufgrund dessen sind hier die derzeiten Koordinaten: %pos%!"),
	ALERT_FIRST_STRIKE_NEVER_ONLINE(ConfigMessageSection.ALERTS, "BOTS_ALERT.firstStrikeNeverOnline", "%player% hat nun einen Strike. Der Strike wurde von %striker% gegeben. Begründung: %reason%\nDer Spieler war noch nicht online und wird an den Spawn-Koordinaten spawnen: %pos%!"),
	ALERT_GAME_STARTED(ConfigMessageSection.ALERTS, "BOTS_ALERT.gameStarted", "%projectname% wurde gestartet!"),
	ALERT_GENERAL_STRIKE(ConfigMessageSection.ALERTS, "BOTS_ALERT.generalStrike", "%player% hat nun den %strikeNumber%ten Strike! Der Strike wurde von %striker% gegeben. Begründung: %reason%"),
	...
	ALERT_SECOND_STRIKE(ConfigMessageSection.ALERTS, "BOTS_ALERT.secondStrike", "%player% hat nun zwei Strikes. Der Strike wurde von %striker% gegeben. Begründung: %reason%\nAufgrund dessen wurde das Inventar geleert!"),
	...
	ALERT_THRID_STRIKE(ConfigMessageSection.ALERTS, "BOTS_ALERT.thirdStrike", "%player% hat nun drei Strikes. Der Strike wurde von %striker% gegeben. Begründung: %reason%\nDamit ist %player% aus %projectname% ausgeschieden!"),
	...
	JOIN_WITH_REMAINING_TIME(ConfigMessageSection.JOINMESSAGE, "joinWithRemainingTime", "%prefix%&a%player% &7hatte den Server zu früh verlassen und hat jetzt noch %colorcode%%seconds% &7Sekunden übrig! Verbleibende &cDisconnects&7: &c%remainingDisconnects%"),
	...
	JOIN_KICK_NO_SESSIONS_LEFT(ConfigMessageSection.KICK, "noSessionLeft", "&cDu hast keine Sessions mehr übrig! %nextLine%&7Warte bis morgen, damit du wieder spielen kannst!"),
	...
	TEAMREQUEST_NO_HASHTAG(ConfigMessageSection.TEAMREQUEST, "noHashtag", "Dein Teamname darf kein '#' enthalten. (Wird automatisch hinzugefügt)"),
	TEAMREQUEST_PLAYER_NOT_ONLINE(ConfigMessageSection.TEAMREQUEST, "playerNotOnline", "%colorcode%%invitor% ist nicht mehr online!"),
	TEAMREQUEST_REVOKED(ConfigMessageSection.TEAMREQUEST, "invationRevoked", "Einladung erfolgreich zurückgezogen!"),
	...
	VARO_COMMANDS_CONFIG_RESET(ConfigMessageSection.VARO_COMMANDS, "config.reset", "&7Alle Einträge wurden erfolgreich zurückgesetzt."),
	...
	VARO_COMMANDS_DISCORD_GETLINK(ConfigMessageSection.VARO_COMMANDS, "discord.getlink", "&7Der Discord Account von %colorcode%%player% heißt %colorcode%%user%&7 und die ID lautet %colorcode%%id%&7!"),
