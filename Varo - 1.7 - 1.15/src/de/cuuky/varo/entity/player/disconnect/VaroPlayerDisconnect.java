...
public class VaroPlayerDisconnect {
	...
	public boolean check() {
		...
		Main.getDataManager().getVaroLoggerManager().getEventLogger().println(LogType.ALERT, Main.getLanguageManager().getValue(LanguageDE.ALERT_DISCONNECT_TOO_OFTEN, null, vp));
		Bukkit.broadcastMessage(Main.getLanguageManager().getValue(LanguageDE.QUIT_TOO_OFTEN, null, vp));
		this.remove();
		return true;
	}
	...
	public static void disconnected(String playerName) {
		...
		scheds.put(playerName, Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
			...
			public void run() {
				...
				Bukkit.broadcastMessage(Main.getLanguageManager().getValue(LanguageDE.QUIT_DISCONNECT_SESSION_END, null, vp).replace("%banTime%", String.valueOf(ConfigSetting.BAN_AFTER_DISCONNECT_MINUTES.getValueAsInt())));
			}
