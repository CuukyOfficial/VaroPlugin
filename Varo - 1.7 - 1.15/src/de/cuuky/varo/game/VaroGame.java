...
public class VaroGame implements VaroSerializeable {
	...
	public void start() {
		...
		if(ConfigEntry.DO_RANDOMTEAM_AT_START.getValueAsInt() > 0) {
			...
			Bukkit.broadcastMessage(Main.getPrefix() + "Alle Spieler haben einen zuf√§lligen Teampartner erhalten!");
		}
		...
		if(ConfigEntry.DO_SPAWN_GENERATE_AT_START.getValueAsBoolean()) {
			...
			Bukkit.broadcastMessage(Main.getPrefix() + "Die L√∂cher f√ºr den Spawn wurden generiert!");
		}
	...
	public void abort() {
		...
		Bukkit.broadcastMessage("ß7Der Start wurde ßcabgebrochenß7!");
	...
	public void end(WinnerCheck check) {
		...
		if(db != null && db.isEnabled()) {
			if(db.getResultChannel() != null && db.isEnabled())
				db.sendMessage((":first_place: " + first + (second != null ? "\n" + ":second_place: " + second : "") + (third != null ? "\n" + ":third_place: " + third : "")) + "\n\nHerzlichen Gl√ºckwunsch!", "Das Projekt ist nun vorbei!", Color.MAGENTA, BotLauncher.getDiscordBot().getResultChannel());
