...
public class InventoryBackupGUI extends SuperInventory {
	...
	public InventoryBackupGUI(Player opener, InventoryBackup backup) {
		super("ßb" + backup.getVaroPlayer().getName(), opener, 9, false);
	...
	public boolean onOpen() {
		linkItemTo(1, new ItemBuilder().displayname("ßaShow").itemstack(new ItemStack(Material.CHEST)).build(), new Runnable() {
		...
		linkItemTo(4, new ItemBuilder().displayname("ß2Restore").itemstack(new ItemStack(Material.EMERALD)).build(), new Runnable() {

			@Override
			public void run() {
				if(!backup.getVaroPlayer().isOnline()) {
					backup.getVaroPlayer().getStats().setRestoreBackup(backup);
					opener.sendMessage(Main.getPrefix() + "Inventar wird beim n√§chsten Betreten wiederhergestellt!");
					return;
				}
		...
		linkItemTo(7, new ItemBuilder().displayname("ßcRemove").itemstack(Materials.REDSTONE.parseItem()).build(), new Runnable() {
