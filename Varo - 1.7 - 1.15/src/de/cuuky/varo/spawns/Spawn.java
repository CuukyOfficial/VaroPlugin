...
public class Spawn implements VaroSerializeable {
	...
	private String getNametagName() {
		return this.player == null ? Main.getLanguageManager().getValue(LanguageDE.SPAWNS_SPAWN_NUMBER).replace("%number%", String.valueOf(number)) : Main.getLanguageManager().getValue(LanguageDE.SPAWNS_SPAWN_PLAYER, null, player).replace("%number%", String.valueOf(number));
	}
