...
public class MessagePlaceholderLoader {
    ...
    private void loadPlayerPlaceholder() {
        ...
        new VaroPlayerMessagePlaceholder("hour", 1, "Ersetzt durch die Stunden der Spielzeit des Spielers", (player) -> ConfigSetting.PLAY_TIME.getValueAsInt() < 1 ? "-" : String.format("%02d", player.getStats().getCountdown() / 3600));
        new VaroPlayerMessagePlaceholder("min", 1, "Ersetzt durch die Minuten der Spielzeit des Spielers", (player) -> ConfigSetting.PLAY_TIME.getValueAsInt() < 1 ? "-" : player.getStats().getCountdownMin(player.getStats().getCountdown()));
        new VaroPlayerMessagePlaceholder("sec", 1, "Ersetzt durch die Sekunden der Spielzeit des Spielers", (player) -> ConfigSetting.PLAY_TIME.getValueAsInt() < 1 ? "-" : player.getStats().getCountdownSec(player.getStats().getCountdown()));
