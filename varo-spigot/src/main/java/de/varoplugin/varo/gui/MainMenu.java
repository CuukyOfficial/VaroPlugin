...
import de.varoplugin.varo.gui.admin.AdminMainMenu;
import de.varoplugin.varo.gui.events.EventListGUI;
import de.varoplugin.varo.gui.items.ItemListSelectInventory;
import de.varoplugin.varo.gui.player.PlayerListChooseGUI;
import de.varoplugin.varo.gui.savable.PlayerSavableChooseGUI;
import de.varoplugin.varo.gui.settings.VaroSettingsMenu;
import de.varoplugin.varo.gui.strike.StrikeListGUI;
import de.varoplugin.varo.gui.team.TeamCategoryChooseGUI;
import de.varoplugin.varo.gui.youtube.YouTubeVideoListGUI;
...
public class MainMenu extends VaroInventory {
    ...
    public void refreshContent() {
        ...
        addItem(18, ItemBuilder.material(XMaterial.COMPASS).displayName("§7Your §5Videos")
                .amount(getFixedSize(VaroPlayer.getPlayer(getPlayer()).getStats().getVideos().size())).build(), (event) ->
                this.openNext(new YouTubeVideoListGUI(getPlayer(), VaroPlayer.getPlayer(getPlayer())))
        );
